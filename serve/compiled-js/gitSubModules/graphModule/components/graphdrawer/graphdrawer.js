...
export default customElements.define('jk224jv-graphdrawer', class jk224jvGraphdrawer extends HTMLElement {
    ...
    #graphAndCanvasDataObjectBuilder(canvas, dataset) {
        ...
        return new GraphAndCanvasData(canvasProperties, graphProperties, dataset, this.#maxNumberOfStepsOnXAxis, this.#numberOfStepsOnYAxis, this.#fontSettings, this.#colorSettings, canvas.getContext('2d'), this.#axisTitles, this.#xAxisLabels);
    }
    setAxisTitles(axisTitles) {
        new Validators().validateAxisTitles(axisTitles);
        const currentAxisTitles = this.axisTitles;
        this.#axisTitles = new AxisTitles(axisTitles.xAxis ?? currentAxisTitles.xAxis, axisTitles.yAxis ?? currentAxisTitles.yAxis);
    }
